...
export default function ClientsPage() {
  ...
  const load = async () => {
    setLoading(true);
    try {
      const r = await fetch(`${API_URL}/clients`, { headers });
      const d = await r.json().catch(() => []);
      if (Array.isArray(d)) setItems(d as Client[]);
    } catch {}
    setLoading(false);
  };
  ...
  const filtered = useMemo(() => {
    ...
    if (!qq) return items;
    return items.filter((x: Client) =>
      [x.name, x.email, x.phone, x.address, x.domisili, x.no_rekening, x.note]
        .filter(Boolean)
        .some(v => String(v).toLowerCase().includes(qq))
    );
  }, [items, q]);

  const resetForm = () => setForm({ id: 0, name: "", email: "", phone: "", address: "", domisili: "", no_rekening: "", note: "", active: true });

  const save = async () => {
    if (!form) return;
    if (!String(form.name || '').trim()) {
      await Swal.fire({ icon: 'warning', title: 'Lengkapi data', text: 'Nama client wajib diisi' });
      return;
    }
    ...
    const isNew = !form.id;
    const url = isNew ? `${API_URL}/clients` : `${API_URL}/clients/${form.id}`;
  ...
  const remove = async (id: number) => {
    const ok = await Swal.fire({ icon: 'warning', title: 'Hapus client ini?', showCancelButton: true, confirmButtonText: 'Hapus', cancelButtonText: 'Batal' }).then(r=>r.isConfirmed);
    if (!ok) return;
    const r = await fetch(`${API_URL}/clients/${id}`, { method: "DELETE", headers });
    if (r.ok) {
  ...
  return (
    <div className="p-4 md:p-6 text-black">
      <h1 className="text-lg md:text-xl font-semibold mb-3">Clients</h1>
      ...
      {form && (
        <div className="rounded-2xl bg-white/80 backdrop-blur border border-white/60 p-4 shadow mb-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            ...
            <label className="flex flex-col gap-1"><span>Telepon</span><input value={form.phone||""} onChange={(e)=>setForm({ ...form, phone: e.target.value })} className="border px-2 py-1 rounded" /></label>
