...
export default function CompaniesPage() {
  ...
  const fetchList = async () => {
    ...
    try {
      ...
      const res = await fetch(`${API_URL}/companies?${params.toString()}`, { headers: { Authorization: `Bearer ${token}` } });
  ...
  useEffect(() => { fetchList(); }, []);
  useEffect(() => { fetchList(); /* eslint-disable-line react-hooks/exhaustive-deps */ }, [page, limit]);
  ...
  const deleteCompany = async (c: Company) => {
    const ok = await Swal.fire({ title: "Hapus?", text: `Hapus ${c.nama}?`, icon: "warning", showCancelButton: true });
        ...
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex items-center gap-2">
            <input
              ...
              placeholder="Cari nama, alamat, email, phone..."
              className="px-3 py-1.5 rounded-lg border border-gray-300 text-sm text-black bg-white"
              aria-label="Cari perusahaan"
            />
            <button onClick={()=>{ setPage(1); fetchList(); }} className="inline-flex items-center gap-2 rounded-lg px-3 py-1.5 text-white bg-indigo-600 hover:bg-indigo-700 shadow" aria-label="Cari" title="Cari">
                ...
                <tbody>
                  {list.map((c) => (
                    <tr key={c.id} className="border-t border-gray-100">
                      <td className="py-2 text-black">{c.nama}</td>
                      <td className="py-2 text-black">{c.label}</td>
                      <td className="py-2 text-black">{c.kota}</td>
                      <td className="py-2 text-black">{c.provinsi}</td>
                      <td className="py-2 text-black">{c.kode}</td>
                      <td className="py-2 text-black">{c.ceo}</td>
                      <td className="py-2 text-black">{c.since}</td>
                      <td className="py-2">
                        <div className="flex items-center gap-2">
