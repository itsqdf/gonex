...
export default function CategoryProdukPage() {
  ...
  const loadData = async () => {
    setLoading(true);
    try {
      const r = await fetch(`${API_URL}/category-products?q=${encodeURIComponent(q)}`, { headers: { Authorization: `Bearer ${token}` } });
  ...
  const createItem = async () => {
    ...
    const r = await fetch(`${API_URL}/category-products`, { method: "POST", headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` }, body: JSON.stringify(payload) });
    if (r.ok) {
  ...
  const startEdit = (it: Item) => {
    setItems(items.map(i => i.id===it.id ? { ...i, _editing: true, _nama: i.nama, _deskripsi: i.deskripsi } : i));
  };
  const cancelEdit = (it: Item) => {
    setItems(items.map(i => i.id===it.id ? { ...i, _editing: false } : i));
  };
  const saveEdit = async (it: Item) => {
    const payload: any = { nama: it._nama, deskripsi: it._deskripsi };
    const r = await fetch(`${API_URL}/category-products/${it.id}`, { method: "PUT", headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` }, body: JSON.stringify(payload) });
    if (r.ok) { loadData(); } else { const d = await r.json().catch(()=>({})); alert(d.error || "Gagal menyimpan"); }
  };
  const deleteItem = async (it: Item) => {
    if (!confirm("Hapus kategori ini?")) return;
    const r = await fetch(`${API_URL}/category-products/${it.id}`, { method: "DELETE", headers: { Authorization: `Bearer ${token}` } });
    ...
    <div className="min-h-screen bg-gradient-to-br from-indigo-50 via-sky-50 to-purple-100 p-6">
      <div className="max-w-5xl mx-auto">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-lg font-semibold text-black">Category Produk</h2>
        ...
        <div className="rounded-2xl bg-white/80 backdrop-blur border border-white/60 p-4 shadow mb-6">
          <h3 className="text-black font-medium mb-2">Tambah Category Produk</h3>
