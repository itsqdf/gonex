...
export default function KasKeluarPage() {
  ...
  const loadData = () => {
    ...
    fetch(`${API_URL}/kas?jenis=KELUAR&page=${page}&limit=${limit}${qs}`, { headers: { Authorization: `Bearer ${token}` } })
      .then(r => r.json().catch(()=>({})))
      .then(data => {
  ...
  useEffect(()=>{ loadData(); /* eslint-disable-next-line */ }, [page, limit, q]);
  ...
  const createItem = async () => {
    ...
    const payload: any = { jenis: "KELUAR", tanggal: iso, jumlah: parseFloat(jumlah||"0"), keterangan };
    ...
    Swal.fire({ title: "Berhasil", text: "Kas Keluar ditambahkan", icon: "success" });
  ...
  const deleteItem = async (it: KasItem) => {
    const ok = await Swal.fire({ title: "Hapus Kas Keluar?", text: `ID ${it.id} akan dihapus.`, icon: "warning", showCancelButton: true, confirmButtonText: "Ya, hapus" });
    ...
    Swal.fire({ title: "Berhasil", text: "Kas Keluar dihapus", icon: "success" });
    loadData();
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-indigo-50 via-sky-50 to-purple-100 p-6">
      <div className="max-w-5xl mx-auto">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-lg font-semibold text-black">Kas Keluar</h2>
        ...
        {showForm && (
          <div className="rounded-2xl bg-white/80 backdrop-blur border border-white/60 p-4 shadow mb-6">
            <h3 className="text-black font-medium mb-2">Tambah Kas Keluar</h3>
